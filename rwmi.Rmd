...
If the imputation model is a joint model for $(Y,R)$ estimated by maximum likelihood, then $D^i$ and $S^{ij}$ are closely related. But if $\psi$ is not estimated by maximum likelihood then $D^i$ describes how it was estimated and $S^{ij}$ describes the density the imputations are taken from.
